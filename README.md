...
> [!IMPORTANT]
> This is an unofficial sandbox repository for developing and testing a UDS RKE2 capability. Please visit the [defenseunicorns](https://github.com/defenseunicorns/uds-rke2) organization for the official UDS RKE2 capability.

This Zarf package provides an air-gapped production environment for deploying [UDS Core](https://github.com/defenseunicorns/uds-core), individual UDS Capabilities, and UDS capabilities aggregated (bundled) via the [UDS CLI](https://github.com/defenseunicorns/uds-cli).
...
This section provides minimal context and instructions for quickly deploying the base UDS RKE2 capability. See [DEVELOPMENT.md](docs/DEVELOPMENT.md) for instructions on how to further develop UDS RKE2.

### Virtual Machines

> [!CAUTION]
> Due to disk formatting, mount operations, networking, and STIG configurations that are applied to a node's host, it is highly recommended that the contents of this repository are not directly installed on a personal machine.
...
[LeapfrogAI](https://github.com/defenseunicorns/leapfrogai), the main supported target of this bundle, requires GPU passthrough to all worker nodes that will have a taint for attracting pods with GPU resource and workload requirements.

Please see the [VM setup documentation](./docs/VM.md) and VM setup scripts to learn more about manually creating a development VM.

### Bundles

There are three main "flavors" of the UDS RKE2 Core bundle, with four distinct flavors in total. Each flavor revolves around the storage and persistence layer of the cluster and comes with its own documentation on configuration and installation, as linked in the bulleted list below. Please refer to that documentation for more details on each bundle flavor's recommendations and capabilities.
...
The following are quick start instructions for the `local-path` flavored UDS RKE2 bundle. This does not include the optional NVIDIA GPU operator and LeapfrogAI workarounds Zarf packages.
...
1. Log in to GitHub Container Registry (GHCR) and [DoD's Registry1](https://registry1.dso.mil/):
...
2. Build all necessary packages, then create and deploy the bundle:
